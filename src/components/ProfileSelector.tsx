...
const ProfileSelector = ({ profiles: initialProfiles, currentProfileId, onSelectProfile }: ProfileSelectorProps) => {
  const [showManagement, setShowManagement] = useState(false);
  const [profiles, setProfiles] = useState<Profile[]>(
    initialProfiles.map(p => ({
      ...p,
      isOwner: p.isOwner ?? (p.id === 1),
          ...
          <div className="grid grid-cols-5 gap-6">
            ...
            {profiles.length < 5 && (
              <button
                ...
                className="group flex flex-col items-center gap-4 p-6 rounded-2xl border-2 border-dashed border-gray-700 hover:border-green-500 transition-all hover:scale-105"
              >
